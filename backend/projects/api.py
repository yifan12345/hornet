...
from projects.api_scheam import ProjectIn
...
def get_project_update(request, project_id: int,payload:ProjectIn):
    """更新项目"""
    project = get_object_or_404(Project,id=project_id)
...
def get_project_delete(request, project_id: int):
    """删除项目"""
    project = get_object_or_404(Project, id=project_id)
...
def project_image_upload(request, file: UploadedFile = File(...)):
    ...
    if suffix not in ["png", "jpg", "jpeg","jmx"]:
